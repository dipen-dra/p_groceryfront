...
export const Chatbot = ({ isVisible, onClose, onConfirmLogout }) => {
    ...
    // --- COMPREHENSIVE DATA FETCHING for BOTH ROLES ---
    const results = useQueries({
        queries: [
            // Admin-specific queries
            {
                queryKey: ['chatbot_dashboardStats'],
                queryFn: () => fetchData('/dashboard/stats', token),
                enabled: isVisible && isAdmin,
            },
            {
                queryKey: ['chatbot_all_orders'],
                queryFn: () => fetchData('/orders', token),
                enabled: isVisible && isAdmin,
            },
            {
                queryKey: ['chatbot_users'],
                queryFn: () => fetchData('/admin/users', token),
                enabled: isVisible && isAdmin,
            },
            // Queries for BOTH admin and user
            {
                queryKey: ['chatbot_products'],
                queryFn: () => fetchData('/products', token),
                enabled: isVisible,
            },
            {
                queryKey: ['chatbot_categories'],
                queryFn: () => fetchData('/categories', token),
                enabled: isVisible,
            },
            // User-specific queries
            {
                queryKey: ['chatbot_my_orders'],
                queryFn: () => fetchData('/orders/myorders', token),
                enabled: isVisible && !isAdmin,
            }
        ],
    });
    ...
    const adminSystemPrompt = `${baseSystemPrompt}
    You are assisting an admin and have access to a full, real-time snapshot of the application's database. Answer questions based on the data provided below. You can perform calculations, summaries, and lookups.

    --- ADMIN DATABASE SNAPSHOT ---
    Dashboard Stats:
    - Total Revenue: ₹${dashboardStatsResult.data?.totalRevenue?.toFixed(2) || '...'}
    - Total Active Orders: ${dashboardStatsResult.data?.totalOrders || '...'}
    - Total Customers: ${dashboardStatsResult.data?.totalCustomers || '...'}
    ...
    Category List (${categoriesResult.data?.length || 0} items):
    ${categoriesResult.data?.map(c => `- ${c.name}`).join('\n') || '...'}

    All Orders (${allOrdersResult.data?.length || 0} items):
    ${allOrdersResult.data?.map(o => `- OrderID: #${o._id.slice(-6)}, Customer: ${o.customer?.fullName}, Status: ${o.status}, Amount: ₹${o.amount}`).join('\n') || '...'}

    User List (${usersResult.data?.length || 0} items):
    ${usersResult.data?.map(u => `- Name: ${u.fullName}, Email: ${u.email}, Joined: ${new Date(u.createdAt).toLocaleDateString()}`).join('\n') || '...'}
    --- END SNAPSHOT ---
    `;

    const userSystemPrompt = `${baseSystemPrompt} 
    You are assisting a customer named ${user?.fullName}. You have access to public store data and this specific user's order history. Help them with their questions. Do NOT mention other users.
    ...
    Product List (${productsResult.data?.length || 0} items):
    ${productsResult.data?.map(p => `- Name: ${p.name}, Price: ₹${p.price}, Category: ${p.category?.name}`).join('\n') || '...'}

    Category List (${categoriesResult.data?.length || 0} items):
    ${categoriesResult.data?.map(c => `- ${c.name}`).join('\n') || '...'}
    
    This User's Order History (${myOrdersResult.data?.length || 0} items):
    ${myOrdersResult.data?.map(o => `- OrderID: #${o._id.slice(-6)}, Status: ${o.status}, Amount: ₹${o.amount}, Date: ${new Date(o.createdAt).toLocaleDateString()}`).join('\n') || 'No orders found for this user.'}
    --- END DATA ---
    `;
    ...
    useEffect(() => {
        if (isVisible) {
            ...
            const suggestions = isAdmin 
                ...
                : ["What's the status of my last order?", "Do you have fresh apples?", "Tell me about Hamro Grocery"];
    ...
    const handleSend = async (messageText = input) => {
        ...
        setInput('');
        const botResponse = await getGeminiResponse(messageText, [...messages, userMessage]);
        setMessages(prev => [...prev, botResponse]);
    };
